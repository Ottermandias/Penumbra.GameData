    ...
    : DataSharer<IReadOnlySet<uint>>(pluginInterface, log, "RacialRestrictedItems", gameData.Language, Version.RestrictedItems, () => CreateItems(log, gameData))
{
    ...
    private static FrozenSet<uint> CreateItems(LunaLogger log, IDataManager gameData)
    {
        ...
        foreach (var item in items.Where(i => i is { EquipRestriction.RowId: > 3, ModelMain: not 0 }))
        {
    ...
    internal (bool Replaced, CharacterArmor Armor) Resolve(CharacterArmor armor, Race race, Gender gender)
    {
        ...
        return (value != quad, new CharacterArmor((ushort)value, (byte)(value >> 16), armor.Stains));
    }
