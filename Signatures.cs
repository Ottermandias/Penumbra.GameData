...
public static class Sigs
{
    ...
    public const string ReadFile         = "48 89 5C 24 ?? 48 89 6C 24 ?? 48 89 74 24 ?? 57 41 54 41 55 41 56 41 57 48 81 EC ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 84 24 ?? ?? ?? ?? 48 63 42";
    ...
    public const string ApricotListenerSoundPlay   = "41 54 41 55 41 56 41 57 48 81 EC ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 4D 8B F9"; // Inlined in 49 89 5B ?? 48 8B 99 ?? ?? ?? ?? F3 0F 11 89  TODO
    ...
    public const string GetEqpIndirect2   = "40 57 48 83 EC ?? 48 8B F9 8B 89 ?? ?? ?? ?? 8B C1";
    public const string SetupVisor        = "E8 ?? ?? ?? ?? 48 8B 8B ?? ?? ?? ?? 0F 57 E4";
    ...
    public const string GetEqpEntry           = "E8 ?? ?? ?? ?? 44 0F B6 9E";
    public const string GetEstEntry           = "8B C1 48 8B 0D ?? ?? ?? ?? 83 EA"; 
